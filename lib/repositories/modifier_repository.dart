...
class ModifierRepository {
  ...
  ModifierRepository({this.token});

  Future<List<Modifier>> fetchModifiersByProductId(
      int productId) async {
    if (productId == 0) {
    ...
    final url =
        '${AppConstants.baseApiPath}/modifiers-addons/get-modifiers-by-product-id?product_id=$productId';
    ...
    try {
      ...
      if (response.statusCode == 200) {
        ...
        final modifiers = jsonData
            .map((item) => Modifier.fromJson(item))
            .toList();
        ...
        AppLogger.error(
            'Access forbidden for productId: $productId');
        return [];
      } else {
        throw Exception(
