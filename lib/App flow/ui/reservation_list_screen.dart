...
class _ReservationListScreenState extends State<ReservationListScreen> {
  ...
  @override
  void dispose() {
    GlobalReservationMonitor().reservationsNotifier.removeListener(
      _reservationListener,
    );
  ...
  Future<void> _fetchReservations({bool isRefresh = false}) async {
    ...
    try {
      final data = await _reservationRepository.fetchAllReservations(
        widget.token,
      );
      ...
      _ReservationListCache.update(_reservations);
    } catch (e) {
      if (!mounted) return;
      ...
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(
          content: Text('Failed to refresh reservations: $e'),
  ...
  bool _isBeforeCutoff(String reservationDate, String cutoffTime) {
    try {
      ...
      final cutoff = DateFormat(
        'yyyy-MM-dd hh:mm a',
      ).parse(fullCutoffDateTimeString);
  ...
  Widget build(BuildContext context) {
    ...
    final currentData =
    filteredReservations.skip(startIndex).take(entriesPerPage).toList();
                            ...
                            decoration: InputDecoration(
                              ...
                              hintStyle: TextStyle(
                                ...
                                isDark
                                    ...
                                    : const Color(0xFFC3C2C2),
                                fontWeight:
                                FontWeight.w400, // change weight here
                                fontSize: 14,
                              ),
                            ),
                          ),
...
// Refresh button
                        ...
                        Container(
                          ...
                          decoration: BoxDecoration(
                            gradient:
                            _userPermissions?.canCreateReservation ?? false
                                ? const LinearGradient(
                          ...
                          child: ElevatedButton(
                            ...
                            onPressed: () {
                              if (_userPermissions?.canCreateReservation ??
                                  false) {
                                Navigator.push(
                                  context,
                                  MaterialPageRoute(
                                ...
                                AreaMovementNotifier.showPopup(
                                  ...
                                  tableName: 'Reservation',
                                  customMessage:
                                  "No permission to create reservation",
                                );
                              }
                            },
                            child: const Row(
...
Widget _pageButton(int page, {bool selected = false, required bool isDark}) {
  return Container(
    width: 28,
    height: 32,
    decoration: BoxDecoration(
